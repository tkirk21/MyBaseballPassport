    ...
    try {
      ...
      if (newLocalImages.length > 0) {
        ...
        if (failedUploads.length > 0) {
          setAlertMessage('One or more photos failed to upload. Please try again.');
          setAlertVisible(true);
          setIsSaving(false);
          return;
        }

        newPhotoUrls = uploadResults.map(r => r.url as string);
      }
      ...
      const docData = {
        ...
        arenaId: match?.arenaId ?? initialData.arenaId ?? null,
      ...
      try {
        ...
        if (err?.code === 'permission-denied') {
          setAlertMessage('Permission denied. You cannot edit this check-in.');
