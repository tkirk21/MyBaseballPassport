  ...
  return (
    <>
      ...
      <Modal visible={addModalVisible} transparent animationType="fade">
        <View style={styles.alertOverlay}>
          <View style={styles.alertContainer}>
            <Text style={styles.alertTitle}>Add Check-In</Text>
            <Text style={styles.alertMessage}>
              Do you want to add this check-in to your profile?
            </Text>
                  ...
                  try {
                    ...
                    const cleanedCompanions = checkin.companions
                      ? checkin.companions
                          ...
                          .map((c: string) => c.trim())
                          .filter((c: string) =>
                            c.toLowerCase() !== `@${currentUser.displayName?.toLowerCase()}`
                          )
                          .join(', ')
                      : '';
                    ...
                    const alreadyExists = existingSnap.docs.some(docSnap => {
                      ...
                      return (
                        data.checkinType === checkin.checkinType &&
                        sameDay &&
                        data.league === checkin.league &&
                        data.arenaName === checkin.arenaName &&
                        data.teamName === checkin.teamName &&
                        data.opponent === checkin.opponent
                      );
                    });
                    ...
                    if (alreadyExists) {
                      ...
                      setAlertMessage('You have already checked into this game.');
                      setAlertVisible(true);
                      return;
                    }
                    ...
                    await setDoc(
                      doc(db, 'profiles', currentUser.uid, 'checkins', `${Date.now()}`),
                      newDoc
                    );
                            ...
                            <View key={index} style={{ marginVertical: 4 }}>
                              {uid ? (
                                <Text
                                  ...
                                  onPress={() => {
                                    if (currentUser && uid === currentUser.uid && !isOwner) {
