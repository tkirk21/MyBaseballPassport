...
export default function FriendsTab() {
      ...
      const unsubActivity = onSnapshot(activityQuery, (snap) => {
        snap.docChanges().forEach(change => {
          ...
          if (change.type === "removed") {
            ...
            setFeed(prev =>
              prev.filter(item =>
                !(item.id === removedId && item.friendId === friendId)
              )
            );
          }
        });
  ...
  const handleAcceptRequest = async (senderId: string) => {
    if (!currentUser) return;
    if (processingRequest === senderId) return;
    ...
    try {
      ...
      setAlertTitle('Unable to Complete Request');
      setAlertMessage('We could not accept this friend request. Please try again.');
      setAlertVisible(true);
    } finally {
  ...
  const handleDenyRequest = async (senderId: string) => {
    ...
    try {
      ...
      setAlertTitle('Unable to Complete Request');
      setAlertMessage('We could not deny this friend request. Please try again.');
      setAlertVisible(true);
    } finally {
      setProcessingRequest(null);
    }
  ...
  const styles = StyleSheet.create({
    ...
    card: { backgroundColor: colorScheme === 'dark' ? '#0D131F' : '#F5F1E6', borderRadius: 12, padding: 16, marginBottom: 30, shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 6, shadowOffset: { width: 0, height: 2 }, elevation: 2, borderWidth: 4, borderColor: '#B22222' },
    ...
    itemText: { fontSize: 16, color: colorScheme === 'dark' ? '#FFFFFF' : '#1D3557' },
    lbAvatar: { width: 36, height: 36, borderRadius: 18, marginRight: 10, },
    lbName: { flex: 1, fontSize: 16, color: colorScheme === 'dark' ? '#FFFFFF' : '#1D3557' },
    ...
    modalCard: { backgroundColor: colorScheme === 'dark' ? '#1D3557' : '#FFFFFF', padding: 20, borderRadius: 16, width: '85%', maxWidth: 340, shadowColor: "#000", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8, elevation: 10 },
    modalTitle: { fontSize: 20, fontWeight: 'bold', marginBottom: 20, color: colorScheme === 'dark' ? '#F5F1E6' : '#1D3557', textAlign: 'center' },
    modalButton: { paddingVertical: 16, alignItems: 'center', borderBottomWidth: 1, borderBottomColor: '#eee', },
    modalButtonText: { fontSize: 17, color: colorScheme === 'dark' ? '#FFFFFF' : '#1D3557', fontWeight: '600' },
    ...
    rankGold: { color: colorScheme === 'dark' ? '#FFFFFF' : '#1D3557', fontWeight: 'bold' },
    searchContainer:{flexDirection:'row',alignItems:'center',borderWidth:1,borderColor:colorScheme==='dark'?'#F5F1E6':'#B22222',backgroundColor:colorScheme==='dark'?'#1D3557':'#FFFFFF',borderRadius:12,paddingHorizontal:12,height:48},
    searchIcon: { marginRight: 10, fontSize: 20, color: colorScheme === 'dark' ? '#F5F1E6' : '#6B7280' },
    searchInput: { flex: 1, fontSize: 16, color: colorScheme === 'dark' ? '#F5F1E6' : '#1D3557', height: '100%' },
    subheading: { fontSize: 20, fontWeight: '600', color: colorScheme === 'dark' ? '#F5F1E6' : '#1D3557', textAlign: 'center', marginBottom: 10 },
    tabActive: { backgroundColor: '#B22222' },
    tabButton:{paddingHorizontal:16,paddingVertical:8,borderRadius:20,backgroundColor:colorScheme==='dark'?'#1D3557':'#FFFFFF'},
    tabText: { fontSize: 14, color: colorScheme === 'dark' ? '#F5F1E6' : '#1D3557', fontWeight: '600' },
    tabTextActive: { color: '#F5F1E6' },
    title: { fontSize: 34, fontWeight: 'bold', color: colorScheme === 'dark' ? '#F5F1E6' : '#1D3557', marginTop: 30, marginBottom: 15, textAlign: 'center', textShadowColor: colorScheme === 'dark' ? '#000000' : '#ffffff', textShadowOffset: { width: 1, height: 1 }, textShadowRadius: 2 },
    unblockText: { color: '#1D3557', fontWeight: 'bold' },
    unknownActivityCard:{backgroundColor:colorScheme==='dark'?'#1D3557':'#FFFFFF',borderRadius:10,padding:14,marginBottom:12},
    unknownActivityText: { fontSize: 15, color: colorScheme === 'dark' ? '#FFFFFF' : "#1D3557" },
            ...
            {pendingRequests.map((req) => {
              ...
              return (
                <View key={req.id} style={styles.listRow}>
                  ...
                  <TouchableOpacity
                    style={[styles.acceptButton, processingRequest === req.id && { opacity: 0.5 }]}
                  ...
                  <TouchableOpacity
                    style={[styles.denyButton, processingRequest === req.id && { opacity: 0.5 }]}
        ...
        {friends.length > 0 && (
          <View style={styles.card}>
            ...
            {allUsers
              ...
              .sort((a, b) => {
                ...
                return bActive - aActive || nameA.localeCompare(nameB);
              })
          ...
          {feed.length === 0 && !loading && (
            <View style={styles.card}>
              ...
              <Text style={styles.emptyText}>
                When your friends check in, root, or banter — it’ll show up here.
              </Text>
            </View>
          )}
